...
fn test_rate_limiter_burst() {
    ...
    ip_limits.insert(Operation::Store, 10);
    ip_limits.insert(Operation::Query, 100);
    ...
    // Fire 10 store checks in rapid succession — all should pass
    for i in 0..10 {
        assert!(
    ...
    // 11th should fail with IpRateLimited
...
fn test_rate_limiter_independent_operations() {
    let mut ip_limits = HashMap::new();
    ip_limits.insert(Operation::Store, 5);
    ip_limits.insert(Operation::Query, 10);
